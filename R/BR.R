...
BR.MCMC <- function(y,x,time,id,n.samples,r.init,delta.init){
  ...
  keep <- matrix(0, n.samples, p + 4+2) #+1 for lambda2 for EN paramter
  ...
  for (s in 1 : n.samples){
    ...
    Var_j <- new_solve(new_solve(tau2*diag(1/lambda2,p))+tmp2);
    #Var_j<- make.positive.definite(Var_j, tol=1e-3)
    Mu_j <- Var_j %*% t(tmp3);
    ...
    tau2_sc <- tau2_scale+(t(as.vector(beta)) %*% diag(lambda2,p)) %*% as.vector(beta)
    ...
    log.can.post2 <- log.post2(Yi,Xi,Wi,beta,alpha,tau2,phi1,phi2)
    ...
    keep[s,] <- c(beta, tau2,phi1,phi2,lambda2,alpha)
  }

  list(beta = keep[ , 1 : p], tau2 = keep[ ,p+1],phi1 = keep[ , (p+2)],phi2 = keep[ , (p+3)],lambda2 = keep[ , p+4],alpha=keep[ , (p+5):(p+6)])
