...
BL.MCMC <- function(y,x,time,id,n.samples,r.init,delta.init){
  ...
  keep <- matrix(0, n.samples, 2 * p + 4+2)
  ...
  for (s in 1 : n.samples){
    ...
    for (j in 1:p)
    {
      Inva2_1 <- sqrt(lambda2 * tau2)/(abs(beta[j]));
      a2[j]   <- 1/func_invGau( Inva2_1, lambda2);
    }

    # update lambda
    lambda2 <-rgamma(1, ncol(x)+r, sum(a2)/2 +delta)
    ...
    keep[s,] <- c(beta, tau2, lambda2, a2,phi1,phi2,alpha)
  }
  # Return the MCMC samples
  list(beta = keep[ , 1 : p], tau2 = keep[ ,p+1], lambda2 = keep[ ,p+2],
       a2 = keep[ , (p+3): (2 * p + 2)],phi1 = keep[ , (2*p+3)],phi2 = keep[ , (2*p+4)],alpha=keep[,(2*p+5):(2*p+6)])
}
