...
BAL.MCMC <- function(y,x,time,id,n.samples,r.init,delta.init){
  ...
  keep <- matrix(0, n.samples, 3*p+3+2)
  ...
  for (s in 1 : n.samples){
    ...
    Var_j <- new_solve(new_solve(tau2*diag(a2))+tmp2);
    Mu_j <- Var_j %*% t(tmp3);
    beta <- rmvnorm(1, Mu_j,Var_j);

    ##########################
    for (j in 1:p)   #5=4+1 unenalized betas plus 1
    {
      a2[j] <- 1/func_invGau(sqrt(lambda2[j] * tau2)/(abs(beta[j])), lambda2[j]);
    }

    # update lambda
    for (j in 1:p)
    {
      lambda2[j]<-rgamma(1, 1+r, a2[j]/2 + delta)
    }
    ...
    tau2_sc <- tau2_scale+(t(as.vector(beta)) %*% new_solve(diag(a2))) %*% as.vector(beta)
    ...
    keep[s,] <- c(beta,tau2,a2,phi1,phi2,lambda2,alpha)
  }
  # Return the MCMC samples
  list(beta = keep[ , 1 : p], tau2 = keep[ ,p+1], a2 = keep[ , (p+2): (2 * p +1)],phi1 = keep[ , (2*p+2)],phi2 = keep[ , (2*p+3)],lambda2 = keep[ ,(2*p+4):(3*p+3)],alpha=keep[ ,(3*p+4):(3*p+5)])
}
